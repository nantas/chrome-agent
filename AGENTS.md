...
**chrome-agent** 是**跨仓库网页抓取服务（cross-repo web scraping service）**，提供标准化、可复用的网页内容获取流程。正式对外入口是 repo-backed 的全局 `chrome-agent` CLI；具体执行仍由当前仓库内部的 `AGENTS.md`、specs、策略库与引擎规则负责。区别于通用的浏览器调试工具仓库，其核心职责是获取和分析网页内容，将浏览器访问与诊断能力作为下层服务的能力层使用。
...
| **crawl** | 基于 strategy 的受边界多页面遍历与批量抓取 | 已验证（bounded） |
...
全局 `chrome-agent` CLI 只是入口层。无论请求从哪个 shell 或 agent 进入，真正的路由判断仍由仓库内规则执行，本节仍是行为权威。
...
全局 `chrome-agent` CLI 不得复制或覆盖上述引擎选择规则；CLI 只负责 repo 解析、dispatch、doctor、clean 和结果归一化。
...
| Global CLI 安装 | `docs/playbooks/chrome-agent-global-install.md` | 全局 `chrome-agent` launcher 安装、doctor 与迁移说明 |
