# Infra Domain: Doctor — Merged Spec

## Source Attribution

| Source Spec | Type | Notes |
|------------|------|-------|
| `doctor-repo-freshness` | modified | `repo_freshness` check, tracked-file change detection, auto-update, installed hash |
| `pipeline-phase-naming` | modified | Phase file naming alignment (doctor-relevant: phase naming affects doctor's file checks) |
...
Define the `chrome-agent doctor` command's repository freshness checking, tracked-file change detection, automatic global file update, and explore Python dependency verification. Also captures the phase file naming convention that affects how doctor validates pipeline file structure.
...
## Phase File Naming (Doctor-Relevant)

### Requirement: phase-file-naming-alignment

Phase 文件 SHALL 按实际功能命名，文件名 SHALL 使用语义化名称而非序号标识。

This affects doctor's ability to validate pipeline file structure.

#### Scenario: phase-file-discovery

- **WHEN** 开发者浏览 `scripts/pipeline/pipeline/phases/` 目录
