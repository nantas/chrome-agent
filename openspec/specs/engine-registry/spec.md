...
- Within Scrapling: lighter engines rank before heavier ones (`scrapling-get` → `scrapling-fetch` → `cloakbrowser-fetch`), with bulk siblings placed adjacent to their non-bulk counterparts when the scenario is batch-oriented
