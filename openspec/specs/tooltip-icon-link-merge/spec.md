# tooltip-icon-link-merge Specification

## Purpose
Provides MediaWiki tooltip pattern preprocessing that merges inline icon+text link pairs into single combined Markdown links. MediaWiki sites (especially wiki.gg) render tooltip-enabled item links as paired `<a>` tags—one containing an icon image, the other containing text—wrapped in `<span class="tooltip">` containers. This spec defines the merge transform that runs before image conversion in the pipeline.
