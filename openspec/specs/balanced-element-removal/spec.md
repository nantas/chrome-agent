# balanced-element-removal Specification

## Purpose
Provides generic HTML balanced-element removal methods that replace non-greedy regex `.*?` for precise nested element deletion. Used by the conversion pipeline to remove TOC divs, edit section spans, nav-box tables, and other nested HTML structures while preserving surrounding content.
