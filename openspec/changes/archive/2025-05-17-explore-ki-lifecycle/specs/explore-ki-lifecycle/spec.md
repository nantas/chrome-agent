...
- Capability: `explore-ki-lifecycle`
- 来源: `proposal.md`
- 变更类型: `added`
- 用户确认摘要: 新增 KI 的分类、优先级、状态追踪和修复验证流程
...
- **WHEN** a self-check failure is caused by a methodology issue in the check itself (regex false match, overly broad counting scope, format mismatch)
...
- **WHEN** a failure cannot be fixed at any of the three owner levels (fundamental engine or tool limitation)
- **THEN** the KI SHALL be classified as owner=`self_check` with status=`open_systemic`
- **THEN** the resolution SHALL document the exact systemic limitation (e.g., "markdownify does not render collapsed template content")
...
- **WHEN** a KI causes infobox field values to be incorrect (wrong data, navigation text leaking into ID fields)
- **OR** a KI causes links or images to be broken/unusable
- **THEN** the KI SHALL be assigned P0

#### Scenario: assign-p1-quality-impact
- **WHEN** a KI causes readability reduction or minor data pollution but does not corrupt key values
- **OR** a self-check false positive wastes debugging time
- **THEN** the KI SHALL be assigned P1

#### Scenario: assign-p2-check-methodology
- **WHEN** a KI is a self-check scope or precision issue that does not affect the actual Markdown output quality
- **THEN** the KI SHALL be assigned P2

#### Scenario: assign-p3-skip-or-cosmetic
- **WHEN** a KI is already in `skip` status (check not applicable to this page type)
- **OR** a KI has negligible visual impact
...
- **WHEN** 3 iterations have been completed
- **AND** KIs still remain unresolved
- **THEN** the agent SHALL present remaining KIs to the user
- **THEN** the user SHALL decide: continue / accept-as-is / adjust-scope
...
- **THEN** the resolution SHALL document: the systemic limitation, why it's not fixable, and any workaround
...
- **THEN** the table SHALL be preceded by `## Known Issues (Post-Validation)`
...
- **THEN** the agent SHALL update the KI table in `strategy.md` to reflect the new status and resolution
...
- **THEN** the agent SHALL re-run sample conversion + self-check
