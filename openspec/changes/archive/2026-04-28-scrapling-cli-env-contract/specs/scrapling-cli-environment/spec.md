...
- Capability: `scrapling-cli-environment`
- 来源: `proposal.md` / 已确认 capabilities
- 变更类型: `new`
- 用户确认摘要: 用户确认——去掉仓库受 git 跟踪文件中的 Scrapling 绝对路径，改用环境变量；所有工作流执行前先检查 Scrapling CLI；若缺失则先确保安装，再询问是否将环境变量写入 `/Users/nantas-agent/.zshenv`
