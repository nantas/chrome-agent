...
- 变更类型: `modified`
- 用户确认摘要: Stage 3 drift 5-6：删除 preprocessor B 轴分支 + 编排逻辑归入共享内核
...
**Reason**: `preprocessor.py` 的 `context` 参数区分 `"explore"`（6 步清洗）和 `"pipeline"`（返回原样）。实际所有调用方（`sample_converter.py` + `convert.py`）均已传入 `"explore"`，"pipeline" 路径为死代码。B 轴差异不应在共享内核内表达。

**Migration**: 调用方移除 `context=` 参数。preprocessor 始终执行 6 步清洗。
...
- **AND** SHALL NOT contain its own `_apply_extraction()` function with orchestrator logic
