...
- `additional_context_refs`:
  ...
  - `docs/governance-and-capability-plan.md` (总体规划文档，Phase 3 定义来源)
  ...
  - `openspec/specs/engine-contracts/spec.md` (引擎契约聚合索引，Phase 2 产出)
  - `openspec/specs/scrapling-get-contract/spec.md` 到 `openspec/specs/chrome-cdp-contract/spec.md` (5 个引擎契约，Phase 2 产出)
  - `sites/` 下现有 5 个站点文件 (迁移源)
  - `docs/playbooks/` 下操作手册 (不重复纳入策略库)

## Source of Truth

- 行为规范真源：`openspec/specs/<capability-id>/spec.md`
  - 本 change 创建的能力：`site-strategy-schema`、`anti-crawl-schema`
- 项目页面角色：上下文输入 / 治理展示 / 结果回写
- 非真源说明：项目页面不得替代 spec delta 作为实现与验证依据
- 策略文件 (`sites/`) 中 YAML frontmatter 为结构化字段的权威来源，`registry.json` 为索引摘要，两者不一致时以 frontmatter 为准
...
- `sites/` 策略文件的新增/修改需同步更新对应的 `registry.json`
