...
- Capability: `pipeline-fetch-phase`
- 来源: `proposal.md` — New Capabilities
- 变更类型: `new`
- 用户确认摘要: 独立的 fetch phase，负责从数据源获取原始内容并写入持久化缓存。支持 manifest 驱动的批量获取、缓存跳过（已有缓存不重复请求）、rate limit 控制和 resume。MediaWiki API 路径和 Scrapling 路径均实现此 phase。
