...
- 变更类型: modified
- 用户确认摘要: 在引擎选择策略、错误矩阵、escalation chain、smoke-check 清单中新增 obscura-fetch 引擎；在 scrapling-get 和 scrapling-fetch 之间插入 cdp_lightweight 类型引擎
...
- **AND** escalation SHALL respect the Scrapling-first principle (all Scrapling + cdp_lightweight engines before full CDP engines)
