...
3. **Scrapling-first default**: For non-API scenarios, start with `scrapling-get` (lightest engine). Escalate only when output is incomplete. The `scrapling-get` extraction stage is **selector-aware**: when the matched strategy declares `extraction.selectors.content`, `fetch` and `crawl` pass it via scrapling `-s` for precise main-content extraction; when no selector is declared they fall back to scrapling's `--ai-targeted` heuristic. This decision is centralized in `buildScraplingExtractionArgs()` (`scripts/lib/scrapling-extraction-args.mjs`). The selector is passed as a discrete argv element (never shell-joined), so special-character selectors like `div[class*='@container/reader-content']` are safe. `cloakbrowser-fetch` is excluded from selector pass-through (its CLI does not accept `-s`) and retains the legacy `--ai-targeted` passthrough.
